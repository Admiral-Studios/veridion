...
type Props = {
  moreData: CompanySearchProductType | null
  onClose: () => void
  preview?: boolean
}

const MoreDataModal: FC<Props> = ({ moreData, onClose, preview }) => {
  return (
    <ComplexFieldsDialog
      open={!!moreData}
      onClose={onClose}
    ...
    >
        ...
        >
          ...
          >
            <Field
              icon='tabler:map-2'
              title='Main Location'
              value={`${moreData?.main_address?.street_number} ${moreData?.main_address?.street}, ${moreData?.main_address?.city}, ${moreData?.main_address?.postcode}, ${moreData?.main_address?.region}, ${moreData?.main_address?.country}`}
            />
            ...
            <Field
              ...
              title='Revenue'
              value={addThousandsDelimiter(moreData?.revenue?.value || 0)}
            />

            <Field icon='mdi:people' title='Employee Count' value={moreData?.employee_count?.value} />
            ...
            <Field
              ...
              title='NAICS'
              value={
                moreData?.naics_2022?.primary
                  ? `${moreData?.naics_2022?.primary.code} - ${moreData?.naics_2022?.primary.label}`
                  : ''
              }
            />
      ...
      {moreData?.search_details?.product_match && (
        <Box
          sx={{
            mt: 4,
            p: 3,
        ...
        >
          ...
          >
            <Field
              ...
              title='Supplier Type'
              value={
                moreData?.search_details?.product_match?.supplier_types?.length
                  ? moreData.search_details.product_match.supplier_types.join(', ')
                  : ''
              }
            />
          ...
          >
            <ListItem>
              <ListItemText
                ...
                secondary={moreData?.search_details?.product_match?.context.headline || ''}
              />
            </ListItem>
            ...
            <ListItem>
              <ListItemText
                ...
                secondary={moreData?.search_details?.product_match?.context.content || ''}
              />
            </ListItem>
            ...
            <ListItem sx={{ pb: 0 }}>
              <ListItemText
                ...
                secondary={
                  moreData?.search_details?.product_match?.context.snippets?.length
                    ? moreData?.search_details?.product_match?.context.snippets.join(', ')
                    : ''
                }
              />
            </ListItem>
